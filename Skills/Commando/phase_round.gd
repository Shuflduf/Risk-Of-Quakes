...
func hold():
	...
	for i in double_tap.guns.size():
		...
		tween.parallel().tween_property(gun, ^"position:y", i * 0.1, 0.4).set_trans(
			Tween.TRANS_BACK
		)
		tween.parallel().tween_property(gun, ^"rotation:x", (-PI * side_mult) / 2.5, 0.2).set_trans(
			Tween.TRANS_EXPO
		)
...
func release():
	...
	for i in double_tap.guns.size():
		...
		(
			tween
			. tween_property(gun, ^"transform", double_tap.og_gun_transforms[i], 0.3)
