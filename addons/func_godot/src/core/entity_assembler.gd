...
func generate_solid_entity_node(
	node: Node, node_name: String, data: _EntityData, definition: FuncGodotFGDSolidClass
) -> Node:
	...
	if data.mesh:
		...
		if (
			not (build_flags & FuncGodotMap.BuildFlags.DISABLE_SMOOTHING)
			and data.is_smooth_shaded(map_settings.entity_smoothing_property)
		):
			mesh_instance.mesh = FuncGodotUtil.smooth_mesh_by_angle(
				data.mesh, data.get_smoothing_angle(map_settings.entity_smoothing_angle_property)
			)
	...
	if "position" in node:
		if node.position is Vector3:
			...
			node.position = (
				Vector2(data.origin.z, -data.origin.y) * map_settings.inverse_scale_factor
			)
...
func generate_point_entity_node(
	node: Node, node_name: String, properties: Dictionary, definition: FuncGodotFGDPointClass
) -> Node:
		...
		if "angles" in properties or "mangle" in properties:
			...
			if angles_raw is String:
				...
				if angles_raw.size() < 3:
					push_error(
						'Invalid vector format for "' + key + '" in entity "' + classname + '"'
					)
					angles_raw = null
			if angles_raw:
		...
		if "scale" in properties:
			...
			if typeof(scale_prop) == TYPE_STRING:
				...
				match scale_arr.size():
					...
					3:
						scale_prop = Vector3(
							scale_arr[1].to_float(),
							scale_arr[2].to_float(),
							scale_arr[0].to_float()
						)
					2:
	...
	if data.definition:
		...
		if def.auto_apply_to_matching_node_properties:
			for property in properties:
				if (
					property == "scale"
					and def is FuncGodotFGDPointClass
					and def.apply_scale_on_map_build
				):
				...
				if property in node:
					if typeof(node.get(property)) == typeof(properties[property]):
						...
						push_error(
							(
								"Entity %s property \'%s\' type mismatch with matching generated node property."
								% [node.name, property]
							)
						)
...
func build(map_node: FuncGodotMap, entities: Array[_EntityData], groups: Array[_GroupData]) -> void:
	var scene_root := (
		map_node.get_tree().edited_scene_root if map_node.is_inside_tree() else map_node
	)
