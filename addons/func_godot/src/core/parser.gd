...
func parse_map_data(map_file: String, map_settings: FuncGodotMapSettings) -> _ParseData:
	...
	var entity_defs: Dictionary[String, FuncGodotFGDEntityClass] = (
		map_settings.entity_fgd.get_entity_definitions()
	)
	...
	for i in range(entities_data.size() - 1, -1, -1):
		...
		if "classname" in entity.properties:
			...
			if classname in entity_defs:
				...
				if (
					not entity.definition is FuncGodotFGDSolidClass
					and not entity.definition is FuncGodotFGDPointClass
				):
					if missing_defs.find(classname) < 0:
						push_error(
							(
								'Invalid entity definition for "'
								+ classname
								+ '". Entity definition must be Solid Class or Point Class.'
							)
						)
				...
				match typeof(prop_default):
					...
					TYPE_VECTOR3:
						...
						if prop_comps.size() > 2:
							properties[property] = Vector3(
								prop_comps[0], prop_comps[1], prop_comps[2]
							)
						else:
							push_error(
								(
									"Invalid Vector3 format for \'"
									+ property
									+ "\' in entity \'"
									+ def.classname
									+ "\': "
					...
					TYPE_VECTOR4:
						...
						if prop_comps.size() > 3:
							properties[property] = Vector4(
								prop_comps[0], prop_comps[1], prop_comps[2], prop_comps[3]
							)
						else:
							push_error(
								(
									"Invalid Vector4 format for \'"
									+ property
									+ "\' in entity \'"
									+ def.classname
									+ "\': "
		...
		var def_properties: Dictionary[String, Variant] = prop_defaults_cache.get(
			def.classname, def.retrieve_all_class_properties()
		)
		var def_descriptions: Dictionary[String, Variant] = prop_descriptions_cache.get(
			def.classname, def.retrieve_all_class_property_descriptions()
		)
		...
		for property in def_properties:
			if not property in properties:
				...
				# Choices
					...
					if (
						prop_desc is Array
						and prop_desc.size() > 1
						and (prop_desc[1] is int or prop_desc[1] is String)
					):
...
func _parse_quake_map(
	map_data: PackedStringArray, map_settings: FuncGodotMapSettings, parse_data: _ParseData
) -> _ParseData:
	var entities_data: Array[_EntityData] = parse_data.entities
	var groups_data: Array[_GroupData] = parse_data.groups
			...
			if brush:
				...
				if ent.properties["classname"] == "func_group" and ent.properties.has("_tb_type"):
					...
					if props["_tb_name"] != "Unnamed":
						group.name = (
							group.name + "_" + (props["_tb_name"] as String).replace(" ", "_")
						)
					...
					match key:
						"plane":
							...
							for i in 3:
								...
								var pts: PackedFloat64Array = tokens[i].split_floats(" ", false)
								var point: Vector3 = (
									Vector3(pts[0], pts[1], pts[2]) * map_settings.scale_factor
								)
								points[i] = point
						...
						"uaxis", "vaxis":
							if brush.faces.size():
								value = value.replace("[", "")
								var vals: PackedFloat64Array = value.replace("]", "").split_floats(
									" ", false
								)
