...
func sample_bezier_surface(
	controls: Array[Vector3], width: int, height: int, u: float, v: float
) -> Vector3:
...
func generate_face_vertices(
	brush: _BrushData, face_index: int, vertex_merge_distance: float = 0.0
) -> PackedVector3Array:
...
func generate_brush_vertices(entity_index: int, brush_index: int) -> void:
	...
	var vertex_merge_distance: float = (
		entity.properties.get(map_settings.vertex_merge_distance_property, 0.0) as float
	)
				...
				if "origin" in entity.properties:
					...
					if origin_comps.size() > 2:
						if entity.origin_type == _OriginType.ABSOLUTE:
							entity.origin = (
								Vector3(origin_comps[0], origin_comps[1], origin_comps[2])
								* map_settings.scale_factor
							)
...
func smooth_entity_vertices(entity_index: int) -> void:
	...
	var smoothing_angle: float = deg_to_rad(
		entity.get_smoothing_angle(map_settings.entity_smoothing_angle_property)
	)
...
func generate_entity_surfaces(entity_index: int) -> void:
		...
		for face: _FaceData in faces:
			...
			if def.add_face_position_metadata:
				for i in num_tris:
					...
					triangle_vertices.assign(
						triangle_indices.map(func(idx: int) -> Vector3: return face.vertices[idx])
					)
			...
			if def.add_collision_shape_to_face_indices_metadata:
				face_index_metadata_map[face] = PackedInt32Array(
					range(current_metadata_index, current_metadata_index + num_tris)
				)
			...
			for i in face.vertices.size():
				...
				var tx_sz: Vector2 = texture_sizes.get(
					face.texture, Vector2.ONE * map_settings.inverse_scale_factor
				)
				arrays[ArrayMesh.ARRAY_TEX_UV].append(
					FuncGodotUtil.get_face_vertex_uv(v, face, tx_sz)
				)
	...
	if entity.is_collision_convex():
		...
		for b in entity.brushes:
			...
			var points := Array(Geometry3D.compute_convex_mesh_points(b.planes)).map(
				op_entity_ogl_xf
			)
			if points.is_empty():
				continue
...
func unwrap_uv2s(entity_index: int, texel_size: float) -> void:
	...
	if (
		entity.mesh
		and entity.is_gi_enabled()
		and not entity.is_smooth_shaded(map_settings.entity_smoothing_property)
	):
...
func build(build_flags: int, entities: Array[_EntityData]) -> Error:
	...
	declare_step.emit(
		"Preparing %s %s" % [entity_count, "entity" if entity_count == 1 else "entities"]
	)
	...
	task_id = WorkerThreadPool.add_group_task(
		generate_entity_vertices, entity_count, -1, false, "Generate Brush Vertices"
	)
	WorkerThreadPool.wait_for_group_task_completion(task_id)

	declare_step.emit("Determining solid entity origins")
	task_id = WorkerThreadPool.add_group_task(
		determine_entity_origins, entity_count, -1, false, "Determine Entity Origins"
	)
	WorkerThreadPool.wait_for_group_task_completion(task_id)

	declare_step.emit("Winding faces")
	task_id = WorkerThreadPool.add_group_task(
		wind_entity_faces, entity_count, -1, false, "Wind Brush Faces"
	)
	WorkerThreadPool.wait_for_group_task_completion(task_id)
	...
	task_id = WorkerThreadPool.add_group_task(
		generate_entity_surfaces, entity_count, -1, false, "Generate Surfaces"
	)
	WorkerThreadPool.wait_for_group_task_completion(task_id)
