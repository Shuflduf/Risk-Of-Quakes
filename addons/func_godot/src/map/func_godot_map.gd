...
enum BuildFlags { UNWRAP_UV2 = 1 << 0, SHOW_PROFILE_INFO = 1 << 1, DISABLE_SMOOTHING = 1 << 2 }  ## Unwrap UV2s during geometry generation for lightmap baking.  ## Print build step information during build process.  ## Force disable processing of vertex normal smooth shading.
...
@export var map_settings: FuncGodotMapSettings = load(
	ProjectSettings.get_setting(
		"func_godot/default_map_settings",
...
@export_flags("Unwrap UV2:1", "Show Profiling Info:2", "Disable Smooth Shading:4")
...
func build() -> void:
	...
	if not map_settings:
		push_warning(
			"Map assembler does not have a map settings provided and will use default map settings."
		)
		load(
			ProjectSettings.get_setting(
				"func_godot/default_map_settings",
	...
	var parse_data: FuncGodotData.ParseData = parser.parse_map_data(
		_map_file_internal, map_settings
	)
