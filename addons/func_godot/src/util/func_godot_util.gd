...
static func create_curve(
	start: Vector3, control: Vector3, end: Vector3, bake_interval: float = 0.05
) -> Curve3D:
	var ret := Curve3D.new()
...
static func update_ref_curve(
	curve: Curve3D, p0: Vector3, p1: Vector3, p2: Vector3, bake_interval: float = 0.05
) -> void:
	curve.clear_points()
...
static func load_texture(
	texture_name: String, wad_resources: Array[QuakeWadFile], map_settings: FuncGodotMapSettings
) -> Texture2D:
	for texture_file_extension in map_settings.texture_file_extensions:
		var texture_path: String = map_settings.base_texture_dir.path_join(
			texture_name + "." + texture_file_extension
		)
		if ResourceLoader.exists(texture_path):
			...
			if texture_file is Texture2D:
				...
				printerr(
					"Error: Texture load failed! (%s) not a valid Texture2D resource", texture_path
				)
...
static func filter_face(texture: String, map_settings: FuncGodotMapSettings) -> bool:
	if map_settings:
		...
		if (
			texture == map_settings.skip_texture
...
static func build_base_material(
	map_settings: FuncGodotMapSettings, material: BaseMaterial3D, texture: String
) -> void:
	...
	for i in pbr_suffixes.size():
		if not pbr_suffixes[i].is_empty():
			...
			if token < 1:
				printerr(
					(
						"No string replacement tokens found in auto-PBR pattern \'"
						+ pbr
						+ "\'! Must have at least one instance of \'%s\' per pattern."
					)
				)
				continue
...
static func build_texture_map(
	entity_data: Array[FuncGodotData.EntityData], map_settings: FuncGodotMapSettings
) -> Array[Dictionary]:
		...
		for brush in entity.brushes:
			for face in brush.faces:
				...
				var material_path: String = (
					map_settings.base_material_dir
					if not map_settings.base_material_dir.is_empty()
					else map_settings.base_texture_dir
				)
				material_path = (
					material_path.path_join(texture_name)
					+ "."
					+ map_settings.material_file_extension
				)
				...
				if ResourceLoader.exists(material_path):
					...
					if material is BaseMaterial3D:
						...
						var albedo = material.get_shader_parameter(
							map_settings.default_material_albedo_uniform
						)
						if albedo is Texture2D:
							texture_sizes[texture_name] = albedo.get_size()
					if not texture_sizes.has(texture_name):
						var texture: Texture2D = load_texture(
							texture_name, wad_resources, map_settings
						)
						if texture:
							texture_sizes[texture_name] = texture.get_size()
					if not texture_sizes.has(texture_name):
						texture_sizes[texture_name] = (
							Vector2.ONE * map_settings.inverse_scale_factor
						)
				...
				# Material generation
					...
					if material is BaseMaterial3D:
						...
						material.set_shader_parameter(
							map_settings.default_material_albedo_uniform, texture
						)
						var path: String = map_settings.base_texture_dir
						for uniform in map_settings.shader_material_uniform_map_patterns.keys():
							if (
								map_settings.shader_material_uniform_map_patterns[uniform].find(
									"%s"
								)
								< 0
							):
								printerr(
									(
										"No string replacement tokens fuond in ShaderMaterial uniform map pattern \'"
										+ map_settings.shader_material_uniform_map_patterns[uniform]
										+ "\'! Must have one instance of \'%s\' per pattern."
									)
								)
								continue
							for texture_file_extension in map_settings.texture_file_extensions:
								var uniform_texture_path: String = (
									(
										map_settings.shader_material_uniform_map_patterns[uniform]
										% [texture_name]
									)
								...
								if ResourceLoader.exists(uniform_texture_path):
									material.set_shader_parameter(
										uniform, load(uniform_texture_path)
									)
									break

					if (
						map_settings.save_generated_materials
						and material
						...
						if not dir:
							...
							dir.make_dir_recursive(
								material_path.get_base_dir().trim_prefix("res://")
							)
...
static func get_quake_uv(
	vertex: Vector3, normal: Vector3, uv_in := Transform2D.IDENTITY, texture_size := Vector2.ONE
) -> Vector2:
	var uv_out: Vector2
...
static func get_face_vertex_uv(
	vertex: Vector3, face: FuncGodotData.FaceData, texture_size: Vector2
) -> Vector2:
...
static func get_quake_tangent(
	normal: Vector3, uv_y_scale: float, uv_rotation: float
) -> PackedFloat32Array:
...
static func smooth_mesh_by_angle(mesh: ArrayMesh, angle_deg: float = 89.0) -> ArrayMesh:
	...
	for surface_index in mesh.get_surface_count():
		...
		var info: Dictionary = {
			"mdt": mdt, "ofs": mesh_vertices.size(), "mat": mesh.surface_get_material(surface_index)
		}
