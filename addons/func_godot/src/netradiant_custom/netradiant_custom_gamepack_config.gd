...
enum NetRadiantCustomMapType { QUAKE_1, QUAKE_3 }  ## Removes PatchDef entries from the map file.  ## Allows the saving of PatchDef entries in the map file.
...
@export var netradiant_custom_shaders: Array[Resource] = [
	preload(
		"res://addons/func_godot/game_config/netradiant_custom/netradiant_custom_shader_clip.tres"
	),
	preload(
		"res://addons/func_godot/game_config/netradiant_custom/netradiant_custom_shader_skip.tres"
	),
	preload(
		"res://addons/func_godot/game_config/netradiant_custom/netradiant_custom_shader_origin.tres"
	)
]
...
func export_file() -> void:
	var game_path: String = (
		FuncGodotLocalConfig.get_setting(FuncGodotLocalConfig.PROPERTY.MAP_EDITOR_GAME_PATH)
		as String
	)
	...
	var gamepacks_folder: String = (
		FuncGodotLocalConfig.get_setting(
			FuncGodotLocalConfig.PROPERTY.NETRADIANT_CUSTOM_GAMEPACKS_FOLDER
		)
		as String
	)
	...
	if DirAccess.open(gamepacks_folder + "/games") == null:
		printerr(
			"Skipping export: No \'games\' folder. Is this the NetRadiant Custom gamepacks folder?"
		)
		return
	...
	target_file_path = (
		gamepacks_folder + "/" + gamepack_name + ".game/scripts/" + gamepack_name + ".shader"
	)
	...
	if file != null:
				...
				if default_build_menu_variables[key] is String:
					...
					push_error(
						(
							...
							% [
								key,
								default_build_menu_variables[key],
		...
		for key in default_build_menu_commands.keys():
			if key is String:
				...
				if default_build_menu_commands[key] is String:
					...
					for command in default_build_menu_commands[key]:
						if command is String:
							...
							push_error(
								(
									"Build option '%s' has invalid command: %s with type: %s; should be: String"
				...
				push_error(
					(
						"Build option '%s' is an invalid type: %s; should be: String"
						% [key, type_string(typeof(key))]
					)
				)
	...
	export_fgd.do_export_file(
		FuncGodotFGDFile.FuncGodotTargetMapEditors.NET_RADIANT_CUSTOM,
		gamepacks_folder + "/" + gamepack_name + ".game/" + base_game_path
	)
