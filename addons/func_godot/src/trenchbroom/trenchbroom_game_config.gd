...
enum GameConfigVersion { Latest, Version4, Version8, Version9 }
...
@export var texture_exclusion_patterns: Array[String] = [
	"*_albedo",
	"*_ao",
	"*_emission",
	"*_height",
...
func _parse_tags(tags: Array) -> String:
	...
	for brush_tag in tags:
		...
		if brush_tag != tags[-1]:
			tags_str += ","
	if (
		game_config_version > GameConfigVersion.Latest
		and game_config_version < GameConfigVersion.Version9
	):
...
func _parse_default_uv_scale(texture_scale: Vector2) -> String:
	...
	return entry_str.format({"x": texture_scale.x, "y": texture_scale.y})
