...
enum WadFormat { Quake, HalfLife }

enum QuakeWadEntryType { Palette = 0x40, SBarPic = 0x42, MipsTexture = 0x44, ConsolePic = 0x45 }

enum HalfLifeWadEntryType { QPic = 0x42, MipsTexture = 0x43, FixedFont = 0x45 }
...
func _get_import_options(path, preset) -> Array[Dictionary]:
	return [
		...
		{"name": "generate_mipmaps", "default_value": true, "property_hint": PROPERTY_HINT_NONE}
	]
...
func _import(source_file, save_path, options, r_platform_variants, r_gen_files) -> Error:
	...
	for entry_idx in range(0, num_entries):
		...
		if (
			(wad_format == WadFormat.Quake and type == int(QuakeWadEntryType.MipsTexture))
			or (wad_format == WadFormat.HalfLife and type == int(HalfLifeWadEntryType.MipsTexture))
		):
			entries.append([offset, in_wad_size, size, type, compression, name_string])
	...
	for texture_data in texture_data_array:
		...
		if wad_format == WadFormat.HalfLife:
			...
			texture_image = Image.create_from_data(
				width, height, false, Image.FORMAT_RGBA8, pixels_rgb
			)
