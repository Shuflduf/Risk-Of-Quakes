...
func do_export_file(
	target_editor: FuncGodotTargetMapEditors = FuncGodotTargetMapEditors.TRENCHBROOM,
	fgd_output_folder: String = ""
) -> void:
	if fgd_output_folder.is_empty():
		fgd_output_folder = (
			FuncGodotLocalConfig.get_setting(FuncGodotLocalConfig.PROPERTY.FGD_OUTPUT_FOLDER)
			as String
		)
	if fgd_output_folder.is_empty():
...
func build_class_text(
	...
	for base_fgd in base_fgd_files:
		if base_fgd is FuncGodotFGDFile:
			...
			printerr(
				"Base Fgd Files contains incorrect resource type! Should only be type FuncGodotFGDFile."
			)
...
func get_fgd_classes() -> Array:
	...
	for cur_ent_def_ind in range(entity_definitions.size()):
		...
		if cur_ent_def == null:
			...
			printerr(
				(
					"Bad value in entity definition set at position %s! Not an entity defintion."
					% cur_ent_def_ind
				)
			)
			continue
		...
		if ent is FuncGodotFGDPointClass or ent is FuncGodotFGDSolidClass:
			...
			for base_class in _generate_base_class_list(entity_def):
				...
				for class_property_desc in base_class.class_property_descriptions:
					class_property_descriptions[class_property_desc] = (
						base_class.class_property_descriptions[class_property_desc]
					)
			...
			for class_property_desc in entity_def.class_property_descriptions:
				class_property_descriptions[class_property_desc] = (
					entity_def.class_property_descriptions[class_property_desc]
				)
	...
	for base_class in entity_def.base_classes:
		if not base_class.classname in visited_base_classes:
			...
			printerr(
				str(
					"Entity '",
					entity_def.classname,
					"' contains cycle/duplicate to Entity '",
