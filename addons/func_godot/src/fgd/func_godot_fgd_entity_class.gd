...
@export var meta_properties: Dictionary[String, Variant] = {
	"size": AABB(Vector3(-8, -8, -8), Vector3(8, 8, 8)), "color": Color(0.8, 0.8, 0.8)
}
...
func build_def_text(
	...
	for prop in meta_props:
		...
		if (
			prop == "model"
			and target_editor != FuncGodotFGDFile.FuncGodotTargetMapEditors.TRENCHBROOM
		):
			continue
		...
		if value is AABB:
			...
			res += "%s %s %s" % [value.r8, value.g8, value.b8]
			...
			if value is Dictionary and class_property_descriptions[prop] is Array:
				...
				if prop_arr.size() > 1 and (prop_arr[1] is int or prop_arr[1] is String):
					var value_str: String = (
						str(prop_arr[1]) if prop_arr[1] is int else '"' + prop_arr[1] + '"'
					)
					...
					printerr(
						(
							str(prop)
							+ " has incorrect description format. Should be [String description, int / String default value]."
						)
					)
			else:
		...
		match typeof(value):
			...
			TYPE_DICTIONARY:
				...
				for choice in value:
					...
					prop_val += (
						"\t\t" + str(choice_val) + ' : "' + choice + '"' + FuncGodotUtil.newline()
					)
				prop_val += "\t]"
			TYPE_ARRAY:
				prop_type = "flags"
				...
				for arr_val in value:
					prop_val += (
						...
						+ str(arr_val[0])
						+ '" : '
						+ ("1" if arr_val[2] else "0")
						+ FuncGodotUtil.newline()
					)
				prop_val += "\t]"
			TYPE_NODE_PATH:
...
func retrieve_all_class_property_descriptions(
	descriptions: Dictionary[String, Variant] = {}
) -> Dictionary[String, Variant]:
