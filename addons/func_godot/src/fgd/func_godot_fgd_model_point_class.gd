...
enum TargetMapEditor { GENERIC, TRENCHBROOM }  ## Entity definition uses the [b]@studio[/b] key word. [member scale_expression] is ignored. Supported by all map editors.  ## Entity definition uses the [b]@model[/b] key word. [member scale_expression] is applied if set.
...
@export_tool_button("Generate GD Ignore File", "FileAccess")
...
func build_def_text(
	target_editor: FuncGodotFGDFile.FuncGodotTargetMapEditors = (
...
func _generate_model() -> void:
	...
	if target_map_editor == TargetMapEditor.TRENCHBROOM:
		...
		if scale_expression.is_empty():
			meta_properties[model_key] = (
				'{"path": "%s", "scale": %s }'
				% [
					_get_local_path(),
					(
						ProjectSettings.get_setting("func_godot/default_inverse_scale_factor", 32.0)
						as float
					)
				]
			)
			...
			meta_properties[model_key] = (
				'{"path": "%s", "scale": %s }' % [_get_local_path(), scale_expression]
			)
	else:
		meta_properties["studio"] = '"%s"' % _get_local_path()

	if generate_size_property:
		meta_properties["size"] = _generate_size_from_aabb(
			gltf_state.meshes, gltf_state.get_nodes()
		)
...
func _get_model_folder() -> String:
	var model_dir: String = (
		ProjectSettings.get_setting("func_godot/model_point_class_save_path", "") as String
	)
