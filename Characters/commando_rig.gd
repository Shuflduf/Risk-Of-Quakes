...
func primary(index: int):
	...
	tween.tween_property(target_hand, ^"position:z", og_pos.z - 0.1, 0.05).set_trans(Tween.TRANS_EXPO)
...
func secondary(started: bool):
	...
	if started:
		for i in hand_handles.size():
			...
			var tween = get_tree().create_tween().set_ease(Tween.EASE_OUT)
			tween.tween_property(hand, ^"position:x", og_pos.x + (0.08 * side_mult), 0.4).set_trans(Tween.TRANS_BACK)
			tween.parallel().tween_property(hand, ^"position:z", og_pos.z - 0.1, 0.4).set_trans(Tween.TRANS_BACK)
